...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        item_dict = {'dialID': dialID, 'turnID': turnID, 'content_arr': content_arr, 'bot_action_idx': bot_action_idx,
...
def collate_fn(data):
    ...
    data_form = {'dailID': item_info['dialID'], 'turnID': item_info['turnID'], 'content_arr': content_arr,
                 'content_arr_len': content_arr_len,
            ...
            if line:
                ...
                if '\t' in line:
                    ...
                    for ent in ent_query.keys():
                        data_item = {'dialID': dialog_counter, 'turnID': system_counter,
                                     'content_arr': content_arr_temp, 'bot_action': bot_action,
                                     'bot_action_idx': bot_action_idx,
                                     'ent_query': [ent, ent_query[ent]], 'ent_query_idx': [ent, ent_query_idx[ent]],
                                     'gold_response': r}
...
def get_seq(pairs, lang, batch_size, type, max_len, query2idx):
    ...
    data_item = {'dialID': dialID_arr, 'turnID': turnID_arr, 'content_arr': content_arr, 'bot_action': bot_action,
                 'bot_action_idx': bot_action_idx,
                 'ent_query': ent_query, 'ent_query_idx': ent_query_idx, 'gold_response': gold_response}
...
def load_img_fea(img_path):
    domains = ['restaurant-dining-room', 'restaurant-kid-friendly', 'restaurant-riverside', 'restaurant-rooftop',
                   'restaurant-rooftop-bar', 'restaurant-with-balcony', 'restaurant-with-bar',
                   'restaurant-with-couches', 'restaurant-with-dance-floor', 'restaurant-with-garden',
                   'restaurant-with-private-room', 'restaurant-with-sea-view']
    image_feas = {}
    for domain in domains:
        domain_fea_path = img_path + "/" + domain + "-fea/train_features.h5"
        fin = h5py.File(domain_fea_path, 'r')
        domain_fea = fin['features'][:]
        if domain not in image_feas:
            image_feas[domain] = domain_fea
    return image_feas

def get_img_fea(img_key, img_feas):
    image_key_list = img_key.split('/')
    image_name, image_domain = image_key_list[-1], image_key_list[-2]
    image_id = image_name.split('.')[0] if image_name.endswith('.png') else -1
    image_fea = img_feas[image_domain][int(image_id)-1]
    return image_fea
